...
module.exports = (app, passport, logger) => {
	...
	app.post('/api/create',passport.authenticate('jwt', {session: false}),(req, res, next) => {
		Jimp.read(req.body.url, function (err, image) {
			if(image!=undefined){
				image.resize(50, 50)            // resize                           
					.write('public/images/thumbnail'+ i +'.'+image.getExtension(),(err) =>{
						if(err){
							res.json({error:true,msg:'Error in downloading'});
						}
						var image1 = 'thumbnail'+i+'.'+this.getExtension();
						i++;
						res.json({error:false,msg:'successfully downloaded',image:image1});
					});
			}
			else{
				res.json({error:true,msg:'Link is not valid'});
